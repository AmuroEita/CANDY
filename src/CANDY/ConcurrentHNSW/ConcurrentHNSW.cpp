/**
 * @Author: Junyao Dong
 * @Date: 10/3/2024, 4:07:40 PM
 * @LastEditors: Junyao Dong
 * @LastEditTime: 10/3/2024, 4:07:40 PM
 * Description: 
 */
...
std::priority_queue<CANDY::HNSW::Node> search_from_candidates_unbounded(
	...
	while (!candidates.empty()) {
		...
		for (auto it = nlist.begin() + begin;
			 it != nlist.end() && it != nlist.begin() + end &&
			 begin < nlist.size();
			 it++) {
			auto v1 = *it;
			if (v1 == nullptr) {
...
void shrink_neighbor_list(
	...
	while (input.size() > 0) {
		...
		for (auto v2 : output) {
			...
			// v1 not ok if there is a neighbor v2 of v1 that is closer to v1
			// than v1 closer to query to prevent v1 getting kicked out
...
int CANDY::HNSW::getLevelsByPtr(INTELLI::TensorPtr idx) { return -1; }
